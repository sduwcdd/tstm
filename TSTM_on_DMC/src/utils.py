...
def write_info(args, fp):
	# Try to get Git version information, if not a Git repository, use 'N/A'
	try:
		git_info = subprocess.check_output(["git", "describe", "--always"]).strip().decode()
	except (subprocess.CalledProcessError, FileNotFoundError):
		# Non-Git repository or Git not installed, use 'N/A' marker
		git_info = 'N/A'
...
class ReplayBuffer(object):
	...
	def sample_svea(self, n=None, pad=4):
		"""Sample a batch and apply random_shift to obs (SVEA style)."""
		idxs = self._get_idxs(n)
		...
		obs = augmentations.random_shift(obs, pad)
		# next_obs is left unshifted; masking will handle it in the algorithm
		return obs, actions, rewards, next_obs, not_dones
