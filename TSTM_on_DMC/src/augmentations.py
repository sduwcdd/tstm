...
def shutdown_places_loader():
    ...
    try:
        if it is not None and hasattr(it, "_shutdown_pinning_thread"):
            it._shutdown_pinning_thread()
    except Exception:
        pass
...
def random_shift(imgs, pad=4):
    ...
    #原实现如下注释，因rebuttal的3090服务器的kornia的cuda存在内存error，改成如下版本
    # _, _, h, w = imgs.shape
    ...
    base_x = torch.arange(w, device=device, dtype=torch.float32).view(1, 1, w).expand(b, h, w)
    base_y = torch.arange(h, device=device, dtype=torch.float32).view(1, h, 1).expand(b, h, w)

    shift_x = torch.randint(-pad, pad + 1, (b, 1, 1), device=device, dtype=torch.int32).to(torch.float32)
    shift_y = torch.randint(-pad, pad + 1, (b, 1, 1), device=device, dtype=torch.int32).to(torch.float32)

    x = base_x + float(pad) + shift_x
    y = base_y + float(pad) + shift_y

    grid_x = (x + 0.5) / float(wp) * 2.0 - 1.0
    grid_y = (y + 0.5) / float(hp) * 2.0 - 1.0
    grid = torch.stack((grid_x, grid_y), dim=-1).to(torch.float32)

    out = F.grid_sample(padded, grid, padding_mode="zeros", align_corners=False)
    return out.to(dtype)
...
def random_overlay_shift(x, alpha=0.5, SRO_l=6, dataset='places365_standard'):
    """Randomly overlay an image from Places with spatial shift across frame stack"""
    global places_iter
    n_stack = x.size(1) // 3
...
def random_crop_shift(x, size=84, n_stack=3, shift_max=6):
    """Vectorized CUDA implementation of random crop with shift across stacked frames"""
    n = x.shape[0]
    c = x.shape[1]
    b = n // n_stack
    img_size = x.shape[-1]
...
def random_overlay_madi(x, method='default', dataset='places365_standard', alpha=0.5, hue=0.0, sat=0.0, val=0.6):
    """Randomly overlay an image from Places - MaDi version with method support"""
    global places_iter
...
def strong_augment(obs, method, overlay_alpha=0.5):
    ...
    if method == 'conv':
        ...
        obs_aug = random_overlay_madi(obs.clone(), method='default', alpha=overlay_alpha)
    elif method == 'splice':
        obs_aug = random_overlay_madi(obs.clone(), method='splice')
    elif method == 'none':
        obs_aug = obs.clone()
    else:
