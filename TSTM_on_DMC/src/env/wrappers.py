...
# modified
# To ensure compatibility with mujoco215
# pip version dm_control >= 1.0.0 (which does not include this repository's custom settings module),
# Here, the pip path takes precedence; if unavailable, it falls back to the locally bundled settings implementation.
try:
    from dm_control.suite.common import settings as dm_settings  # An exception will be raised if the pip version does not exist.
except Exception:
    from env.dm_control.dm_control.suite.common import settings as dm_settings  # Local rollback
...
class ColorWrapper(gym.Wrapper):
    ...
    def reload_physics(self, setting_kwargs=None, state=None):
        ...
        self._reload_physics(
            *dm_settings.get_model_and_assets_from_setting_kwargs(
