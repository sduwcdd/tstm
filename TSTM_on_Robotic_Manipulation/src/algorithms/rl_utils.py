...
def compute_attribution_mask(obs_grad, quantile=0.95, n_frames=None):
    """
    计算attribution mask，支持任意帧数
    Args:
        obs_grad: shape [B, C, H, W], 其中C = n_frames * 3
        quantile: 保留的像素分位数
        n_frames: 帧数，如果为None则自动推断
    """
    mask = []
    B, C, H, W = obs_grad.shape
    # 自动推断帧数
    if n_frames is None:
        n_frames = C // 3
    ...
    for i in range(n_frames):
        frame_start = i * 3
        frame_end = (i + 1) * 3
        attributions = obs_grad[:, frame_start:frame_end].abs().sum(dim=1)
        q = torch.quantile(attributions.flatten(1), quantile, 1)
        mask.append((attributions >= q[:, None, None]).unsqueeze(1).repeat(1, 3, 1, 1))
