...
def _get_out_shape_feat_encoder(in_shape, layers, layers2):
    x = torch.randn(*in_shape).unsqueeze(0)  # (1, C_total, H, W)
    if len(x.shape) == 4:
        B, C_total, H, W = x.shape
        channels_per_frame = 3
        num_frames = C_total // channels_per_frame
        
        x = x.reshape(B * num_frames, channels_per_frame, H, W)
        x = layers(x)
        
        _, C_out, H_new, W_new = x.shape
        x = x.reshape(B, num_frames * C_out, H_new, W_new)
...
#modified for simgrl, report frame_stack=2 in paper
class SharedCNN_feat_stack(nn.Module):
    def __init__(self, obs_shape, num_layers=11, num_filters=32):
        ...
        self.layers = [
            CenterCrop(size=84),
            NormalizeImg(),
            nn.Conv2d(3, num_filters1, 3, stride=2)
        ]
        for _ in range(1, num_layer1):
            self.layers.append(nn.ReLU())
            self.layers.append(nn.Conv2d(num_filters1, num_filters1, 3, stride=1))
        
        input_channels_stage2 = num_filters1 * num_frames
        self.layers2 = []
        self.layers2.append(nn.ReLU())
        self.layers2.append(nn.Conv2d(input_channels_stage2, num_filters, 3, stride=1))
